package com.example.lab_07.ui.categories.view

import androidx.compose.foundation.Image
import androidx.compose.foundation.clickable
import androidx.compose.foundation.layout.Column
import androidx.compose.foundation.layout.Row
import androidx.compose.foundation.shape.RoundedCornerShape
import androidx.compose.material3.OutlinedCard
import androidx.compose.runtime.Composable
import androidx.compose.foundation.layout.fillMaxWidth
import androidx.compose.foundation.layout.padding
import androidx.compose.foundation.layout.size
import androidx.compose.material3.Card
import androidx.compose.material3.MaterialTheme
import androidx.compose.material3.Text
import com.example.lab_07.networking.response.categories.Categories
import androidx.compose.ui.Modifier
import androidx.compose.ui.res.stringResource
import androidx.compose.ui.unit.dp
import androidx.navigation.NavController
import coil.compose.rememberAsyncImagePainter
import com.example.lab_07.R
import com.example.lab_07.database.categories.MealCategoryEntity
import com.example.lab_07.navigation.NavigationState
import com.example.lab_07.navigation.navigateTo

@Composable
fun CardMealCategory(meal :MealCategoryEntity, navController: NavController){
    OutlinedCard(
        shape = RoundedCornerShape(8.dp),
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 16.dp)
    ){
        Row{
            Image(
                painter = rememberAsyncImagePainter(meal.imageUrl),
                contentDescription = null,
                modifier = Modifier
                    .size(88.dp)
                    .padding(4.dp)
            )

            Column(
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(16.dp).clickable {
                        meal.name?.let {
                            navigateTo(
                                navController,
                                NavigationState.MealsRecipesList.createRoute(it)
                            )
                        }
                    }
            ){
                Text(
                    text = stringResource(R.string.category_name),
                    style = MaterialTheme.typography.titleSmall
                )
                Text(
                    text = meal.name ?: "",
                    style = MaterialTheme.typography.titleLarge
                )
                Text(
                    text = stringResource(R.string.date_content, 11, 2020),
                    style = MaterialTheme.typography.bodySmall
                )
            }
        }

    }
}